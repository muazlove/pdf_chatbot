...
# Assuming you've set the API key in environment variables
...
def get_vector_store(text_chunks):
    ...
    # Check for empty text chunks and handle gracefully
    if not text_chunks:
        st.error("No text found in the uploaded PDFs. Please try uploading PDFs with content.")
        return None

    try:
        # Attempt to generate embeddings
        embeddings_list = embeddings.encode(text_chunks)
    except Exception as e:
        st.error(f"Error generating embeddings: {e}")
        return None

    # Handle cases with a single embedding (adjust if necessary)
    if len(embeddings_list) == 1:
        index = FAISS.IndexFlatIP(embeddings_list[0].shape[0])  # Use IndexFlatIP for single embedding
    else:
        index = FAISS.IndexFlatL2(embeddings_list[0].shape[0])  # Use IndexFlatL2 for multiple embeddings

    vector_store = FAISS(index)
    vector_store.add(embeddings_list)
...
def user_input(user_question):
    ...
    # Check if vector store exists (optional)
